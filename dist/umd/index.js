...
class IIIFLayer extends leaflet__WEBPACK_IMPORTED_MODULE_0__.TileLayer {
  ...
  constructor(url, options = {}) {
    super(url, options); // Leaflet map

    this.map = null; // Server capabilities

    this.server = _types__WEBPACK_IMPORTED_MODULE_1__.SERVER_CAPABILITIES_DEFAULT; // Dimension of the image
  ...
  initialize(url, options) {
    ...
    this.initializePromise = new Promise((resolve, reject) => __awaiter(this, void 0, void 0, function* () {
      try {
        ...
        const data = yield response.json(); // saving the image dimension

        this.height = data.height;
        this.width = data.width; // server capabilities

        this.server = (0,_utils_server_capabilities__WEBPACK_IMPORTED_MODULE_3__.computeServerCapabilities)(data); // Settings

        this.options = leaflet__WEBPACK_IMPORTED_MODULE_0___default().Util.setOptions(this, Object.assign({}, _types__WEBPACK_IMPORTED_MODULE_1__.DEFAULT_OPTIONS, // Server pref
        {
  ...
  onAdd(map) {
    this.map = map; // Wait for info.json fetch and parse to complete

    this.initializePromise.then(() => {
      // Compute image sizes
      this.computeZoomLayers(); // calling super after we compute all the zoom

      super.onAdd(map); // Fit bounds
  ...
  onRemove(map) {
    // calling super
    super.onRemove(map); // Remove maxBounds set for this image
  ...
  getTileUrl(coords) {
    ...
    if (!zoomLayer) throw new Error(`Can't create tile for zoom ${z}`); // Handle rotation
    ...
    const tileSizeY = this.options.tileSize.y / zoomLayer.scale; // Compute the image region / bbox NW/SE
    ...
    const maxY = Math.min(minY + tileSizeY, this.height); // In mirroring, if we do the diff with the width,
  ...
  _isValidTile(coords) {
    ...
    const x = coords["x"];
    const y = coords["y"];
    const z = coords["z"]; // Handle rotation (leaflet axis are reversed, so a 90 rotation is in fact a -90 rotation)
    ...
    if (this.options.minZoom <= z && z <= this.options.maxZoom) {
      ...
      if (0 <= unprojectedSquare.bottomLeft.x && unprojectedSquare.bottomLeft.x < originalZoomLayer.tiles[0] && 0 <= unprojectedSquare.bottomLeft.y && unprojectedSquare.bottomLeft.y < originalZoomLayer.tiles[1]) isValid = true;
    }
  ...
  getBounds() {
    ...
    if (this.map !== null && originalZoomLayer) {
      const projectedSquare = (0,_utils_projection__WEBPACK_IMPORTED_MODULE_5__.projectSquare)(this.options.rotation, {
        bottomLeft: {
          x: 0,
          y: originalZoomLayer.height
        },
        topRight: {
          x: originalZoomLayer.width,
          y: 0
        }
      });
  ...
  changeRotation(value) {
    this.options.rotation = value; // Set max bound
...
function computeServerCapabilities(data) {
  let uri = `${data["@context"]}`; // in v3 it can be an array where the last item is the uri
...
function computeServerCapabilitiesForV1(data) {
  ...
  capabilities.version = "1.1"; // Search the level compliance level

  const match = `${data.profile}`.match(/^http:\/\/library.stanford.edu\/iiif\/image-api\/1.1\/compliance.html#(.*)$/);
  const level = match && match.length === 2 ? match[1] : "level0"; // Defined capabilities per level
  ...
  if (data.formats) capabilities.formats = data.formats; // Qualities

  if (data.qualities) capabilities.qualities = data.qualities; // Tiles size

  if (data.tile_width) capabilities.tileSize = leaflet__WEBPACK_IMPORTED_MODULE_0___default().point(data.tile_width, data.tile_height ? data.tile_height : data.tile_width); // Scale factors

  if (data.scale_factors && data.scale_factors.length > -1) {
    capabilities.minZoom = Math.log2(data.scale_factors[data.scale_factors.length - 1]) * (data.scale_factors[data.scale_factors.length - 1] > 1 ? -1 : 1);
...
function computeServerCapabilitiesForV2(data) {
  ...
  const level = data.profile[0]; // Defined capabilities per level
  ...
  if (data.profile[1] && data.profile[1].formats) capabilities.formats = data.profile[1].formats; // Qualities

  if (data.profile[1] && data.profile[1].qualities) capabilities.qualities = data.profile[1].qualities; // Rotation

  if (data.profile[1] && data.profile[1].supports && (data.profile[1].supports.includes("rotationBy90s") || data.profile[1].supports.includes("rotationArbitrary"))) capabilities.rotation = true; // Mirroring

  if (data.profile[1] && data.profile[1].supports && data.profile[1].supports.includes("mirroring")) capabilities.mirroring = true; // Tiles size & scale factors
  ...
  if (data.tiles && data.tiles.length > -1) {
    ...
    capabilities.minZoom = Math.log2(tile.scaleFactors[tile.scaleFactors.length - 1]) * (tile.scaleFactors[tile.scaleFactors.length - 1] > 1 ? -1 : 1);
...
function computeServerCapabilitiesForV3(data) {
  ...
  const level = data.profile; // Defined capabilities per level
  ...
  if (data.extraFormats && data.extraFormats instanceof Array) capabilities.formats = capabilities.formats.concat(data.extraFormats.filter(ef => !capabilities.formats.includes(ef))); // Extra qualities

  if (data.extraQualities && data.extraQualities instanceof Array) capabilities.qualities = capabilities.qualities.concat(data.extraQualities.filter(eq => !capabilities.qualities.includes(eq))); // Rotation

  if (data.extraFeatures && (data.extraFeatures.includes("rotationBy90s") || data.extraFeatures.includes("rotationArbitrary"))) capabilities.rotation = true; // Mirroring

  if (data.extraFeatures && data.extraFeatures.includes("mirroring")) capabilities.mirroring = true; // Tiles size
...
class IIIFControl extends leaflet__WEBPACK_IMPORTED_MODULE_0__.Control {
  ...
  onAdd(map) {
    const container = this.getContainer(); // Waiting the init of the layer
    ...
    this.layer.initializePromise.then(() => {
      ...
      if (this.options.quality.enabled === true) {
        ...
        this.createActions(container, this.options.quality.title, `${CONTROL_NAME}-quality`, this.options.quality.html, qualities.map(quality => {
          return {
            ...
            innerHTML: quality,
            fn: () => {
              map.fire(_event__WEBPACK_IMPORTED_MODULE_1__.IIIF_EVENTS.CHANGE_QUALITY, {
                value: quality
              });
            }
          };
      ...
      if (this.options.format.enabled === true) {
        ...
        this.createActions(container, this.options.format.title, `${CONTROL_NAME}-format`, this.options.format.html, formats.map(format => {
          return {
            ...
            innerHTML: format,
            fn: () => {
              map.fire(_event__WEBPACK_IMPORTED_MODULE_1__.IIIF_EVENTS.CHANGE_FORMAT, {
                value: format
              });
            }
          };
      ...
      if (this.options.rotation.enabled === true && this.layer.server.rotation === true) {
        ...
        this.createActions(container, this.options.rotation.title, `${CONTROL_NAME}-rotation`, this.options.rotation.html, rotations.map(rotation => {
          return {
            ...
            innerHTML: rotation,
            fn: () => {
              map.fire(_event__WEBPACK_IMPORTED_MODULE_1__.IIIF_EVENTS.CHANGE_ROTATION, {
                value: Number.parseInt(rotation)
              });
            }
          };
        }));
      ...
      if (this.options.mirroring.enabled === true && this.layer.server.mirroring === true) {
        this.createButton(container, this.options.mirroring.title, `${CONTROL_NAME}-mirroring`, this.options.mirroring.html, () => {
          map.fire(_event__WEBPACK_IMPORTED_MODULE_1__.IIIF_EVENTS.CHANGE_MIRRORING, {
            value: !this.layer.options.mirroring
          });
        });
      }
    });
  ...
  createActions(container, title, className, innerHTML, actions) {
    ...
    this.createButton(actionsWrapper, title, className, innerHTML, () => {
      if (container.dataset.opened === className) container.dataset.opened = "";else container.dataset.opened = className;
    });
...
window.L["Iiif"] = {
  Event: _event__WEBPACK_IMPORTED_MODULE_0__.IIIF_EVENTS
};
